...
The agent may select a registered tool and provide fields declared by that tool. Credentials,
policy, adapter selection, and pending-call approval remain under host authority.
...
requests after `allow` or presentation of a valid approval receipt.
...
Successful adapter calls append declared effects to the session history. A per-session lock makes
evaluation and effect recording atomic for concurrent calls.
...
the receipt under the pending-state lock. It then checks the current session history under
...
`ApprovalAuthority` interface and keep its key in a managed secret store.

## Credential handling

Tool registration can declare a credential alias. The host resolves that alias after policy and
approval checks, then supplies the value through `ExecutionContext`. The host selects the alias.
Audit records omit the execution context and adapter output.
...
store supplied by the host application.

## Policy acceptance

Paired policy contracts exercise a prohibited request and a permitted counterpart through the same
policy. The pair records every semantic field that changes. Contract construction requires all
other request fields to match.
...
traces whose latest runtime decision is `allow`, matching histories produced before approval. The
search stops at the supplied event depth and decision budget.

`FlowRequirement` supplies the expected effect-to-sink relation through a separate data model. The
oracle reads successful events independently of the runtime sequence matcher. Reports include
...
A branch continues whenever either policy allows its latest action. Decisions retain separate
reachability flags, which prevents proposal-only paths from disappearing behind an earlier policy
difference.

`FlowRequirement` is the sole source of expected controls and evidence. The
report records old-policy defects, proposal defects, regressions, corrections, and uncovered
requirements separately. Its digest includes every explored decision. The default rendering keeps
the case-level table optional.
...
trajectory contracts used by the runtime policy evaluator. The policy remains immutable during
synthesis.
